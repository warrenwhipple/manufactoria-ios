...
class LevelEditData: NSObject, NSCoding {
  let grid: Grid
  ...
  init(levelNumber: Int) {
    ...
    let filePath = LevelEditData.filePathForLevelNumber(levelNumber)
    if NSFileManager.defaultManager().fileExistsAtPath(filePath) {
      if let levelData = NSKeyedUnarchiver.unarchiveObjectWithFile(filePath) as? LevelEditData {
  ...
  func saveWithLevelNumber(levelNumber: Int) {
    NSKeyedArchiver.archivedDataWithRootObject(self).writeToFile(LevelEditData.filePathForLevelNumber(levelNumber), atomically: true)
  }
